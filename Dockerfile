FROM ubuntu:18.04
MAINTAINER Mohamed BEN ALI <mohamed.benali91@yahoo.fr>
WORKDIR /root
# install java 8  &  scala & open ssh   
RUN apt-get update && apt-get install -y openssh-server openjdk-8-jdk wget software-properties-common vim nano git

# install pip3 & jupyter notebook
...
ENV PYTHONPATH=$PYTHONPAH:/usr/lib/python3.6
